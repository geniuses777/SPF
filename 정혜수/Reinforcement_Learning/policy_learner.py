...
class PolicyLearner:
    ...
    def fit(
        self, num_epoches=1000, max_memory=60, balance=10000000,      # balance : 초기 자본금 설정
        discount_factor=0, start_epsilon=.5, learning=True):          # 투자 시뮬레이션만 하려면 False로 변경  # True : 정책 신경망 모델 생성
        ...
        epoch_summary_dir = os.path.join(
            settings.BASE_DIR, 'epoch_summary/%s/epoch_summary_%s' % (
                self.stock_code, settings.timestr))     # timestr : 날짜, 시간
