...
# 투자금과 보유 주식을 관리하기 위한 클래스
class Agent:
    # 에이전트 상태가 구성하는 값 개수
    STATE_DIM = 2  # 주식 보유 비율, 포트폴리오 가치 비율 (에이전트 상태)
    ...
    # 에이전트 상태 반환
    def get_states(self):
        self.ratio_hold = self.num_stocks / int(                                         # 1: 최대 보유할 수 있는 주식 0 : 하나도 X
            self.portfolio_value / self.environment.get_price())
        self.ratio_portfolio_value = self.portfolio_value / self.base_portfolio_value    # 수익이 발생했는지 손실이 발생했는지 판단
        return (
            self.ratio_hold,
