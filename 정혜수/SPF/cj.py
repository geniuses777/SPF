...
try:
    with connection.cursor() as cursor:
        sql = "select accuracy from stock_hye WHERE company_name='CJ'"
...
plt.savefig("C:\source\SPF\chart_picture\cj.png", dpi=300)
#plt.show()
...
try:
    with connection.cursor() as cursor:
        ...
        cursor.execute(sql, ('CJ', int(predict), image))          # 넣으려는 값
    connection.commit()
