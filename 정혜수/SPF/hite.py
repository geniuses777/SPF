...
try:
    with connection.cursor() as cursor:
        sql = "select accuracy from stock_hye WHERE company_name='하이트진로'"
...
plt.title('[하이트진로] 예측 그래프\n예측값 : %d \t\t\t\t\t\t\t\t오차율 : %s' % (predict, accuracy), fontproperties=fontprop)

#plt.savefig("./chart_picture/hite.png",dpi=300)
plt.savefig("C:\source\SPF\chart_picture\hite.png", dpi=300)
#plt.show()
...
try:
    with connection.cursor() as cursor:
        ...
        image = convertToBinaryData("C:\source\SPF\chart_picture\hite.png")
        cursor.execute(sql, ('하이트진로', int(predict), image))          # 넣으려는 값
    connection.commit()
