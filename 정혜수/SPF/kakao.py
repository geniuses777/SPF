...
try:
    with connection.cursor() as cursor:
        sql = "select accuracy from stock_hye WHERE company_name='카카오'"
...
try:
    with connection.cursor() as cursor:
        ...
        cursor.execute(sql, ('카카오', int(predict), image))          # 넣으려는 값
    connection.commit()
